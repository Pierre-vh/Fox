...
bool ParserTests::runTests(std::ostream & out,const bool& condensed, const bool& showindividualtestdetails)
{
	showIndividualTestDetails = showindividualtestdetails;
...
bool ParserTests::runIndividualTest(const IndividualTest & curtest, std::ostream & out, const bool & condensed)
{
	...
	for (auto test : tests_strs)
	{
		...
		if (curtest.testfn(par, showIndividualTestDetails) && ctxt.isSafe())
		{
		...
		else
		{
			...
			else
			{
				if (!condensed)
				{
					...
					if (showIndividualTestDetails)
					{
						std::string logs = ctxt.getLogs();
						if (logs.size())
						{
							out << indent(5) << "Logs for this test:\n";
							out << ctxt.getLogs();
						}
					}
				}
